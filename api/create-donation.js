const { createMollieClient, SequenceType } = require('@mollie/api-client');

export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const apiKey = process.env.MOLLIE_API_KEY;
  const siteUrl = process.env.PUBLIC_SITE_URL;

  if (!apiKey || !siteUrl) {
    console.error('[create-donation] missing env vars - MOLLIE_API_KEY:', !!apiKey, 'PUBLIC_SITE_URL:', !!siteUrl);
    return res.status(500).json({ error: 'Mollie env vars not configured' });
  }

  const body = req.body;
  const { amount, recurring, donorName = '', donorEmail = '', giftAid = false } = body || {};

  const amountNum = parseFloat(amount);
  if (!Number.isFinite(amountNum) || amountNum < 1) {
    return res.status(400).json({ error: 'Amount must be at least £1' });
  }
  if (recurring && (!donorName || !donorEmail)) {
    return res.status(400).json({ error: 'Name and email required for recurring' });
  }
  if (donorEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(donorEmail)) {
    return res.status(400).json({ error: 'Invalid email' });
  }

  try {
    const mollie = createMollieClient({ apiKey });
    const formattedAmount = amountNum.toFixed(2);
    const redirectUrl = `${siteUrl.replace(/\/$/, '')}/donate/thank-you`;

    let payment;
    if (recurring) {
      const customer = await mollie.customers.create({
        name: donorName,
        email: donorEmail,
        metadata: { source: 'astro' },
      });
      payment = await mollie.payments.create({
        amount: { currency: 'GBP', value: formattedAmount },
        description: `Level Up — Monthly donation (£${formattedAmount}/month)`,
        redirectUrl,
        customerId: customer.id,
        sequenceType: SequenceType.first,
        metadata: {
          type: 'recurring',
          amount: formattedAmount,
          donorEmail,
          donorName,
          giftAid: String(giftAid),
          source: 'astro',
        },
      });
    } else {
      payment = await mollie.payments.create({
        amount: { currency: 'GBP', value: formattedAmount },
        description: `Level Up — Donation (£${formattedAmount})`,
        redirectUrl,
        metadata: {
          type: 'one-time',
          amount: formattedAmount,
          donorEmail: donorEmail || null,
          donorName: donorName || null,
          giftAid: String(giftAid),
          source: 'astro',
        },
      });
    }
    
    res.status(200).json({ checkoutUrl: payment.getCheckoutUrl() });
  } catch (err) {
    console.error('[create-donation] Mollie error', err);
    res.status(500).json({ error: 'Failed to create payment' });
  }
};
